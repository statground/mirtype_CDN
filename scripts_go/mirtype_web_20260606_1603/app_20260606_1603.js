(function () {
  ...
  function init() {
    ...
    elements.languageControls = document.querySelector("[aria-label='언어']");
  ...
  function bindButtons() {
    ...
    document.querySelectorAll("[data-language]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        syncActiveButton("[data-language]", state.language);
        applyLocale();
        renderKeyboard();
        resetPractice();
      });
    ...
    document.querySelectorAll("[data-duration]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        syncActiveButton("[data-duration]", String(state.duration));
        resetPractice();
      });
    });
  ...
  function syncActiveButton(selector, value) {
    document.querySelectorAll(selector).forEach(function (button) {
      var buttonValue = button.dataset.mode || button.dataset.language || button.dataset.lesson || button.dataset.duration || button.dataset.game;
      button.classList.toggle("is-active", buttonValue === value);
    });
  }

  function locale() {
    return locales[state.language] || locales.ko;
  }
  ...
  function applyLocale() {
    ...
    elements.languageControls.setAttribute("aria-label", copy.aria.language);
    ...
    setButtonText("[data-duration]", copy.durations);
  ...
  function resetPractice() {
    ...
    elements.input.disabled = state.mode === "keyboard";
    elements.resultPanel.hidden = true;
    var copy = locale();
    elements.prompt.classList.toggle("is-long", state.mode === "sentence");
    elements.gameStage.classList.toggle("is-active", state.mode === "game");
    elements.prompt.hidden = state.mode === "game";
    elements.input.hidden = state.mode === "keyboard";
    elements.lessonControls.hidden = state.mode !== "seat";
    elements.durationControls.hidden = state.mode === "game" || state.mode === "keyboard";
    elements.gameControls.hidden = state.mode !== "game";
    ...
    elements.timeLabel.textContent = state.mode === "game" ? copy.timeLeft : copy.time;
  ...
  function getKeyboardTitle() {
    var copy = locale();
    return copy.keyboardName || data[state.language].label + " " + copy.keyboardSuffix;
  }
  ...
  function buildTarget() {
    ...
    if (state.mode === "word") {
      state.units = sample(data[state.language].words, 36);
      return state.units.join(" ");
    }
  ...
  function buildSentenceTarget() {
    var languageData = data[state.language];
    var shortCount = state.duration === 0 || state.duration >= 120 ? 8 : state.duration >= 60 ? 6 : 4;
    var longCount = state.duration === 0 || state.duration >= 120 ? 2 : state.duration >= 60 ? 1 : 0;
  ...
  function buildSeatUnits() {
    var lessonSet = data[state.language].seat[state.lesson] || data[state.language].seat.mixed;
    var units = [];
    for (var i = 0; i < 5; i += 1) {
  ...
  function updatePractice() {
    if (state.mode === "keyboard" || state.finished) {
      return;
    }
    if (state.mode === "game") {
      updateGameInput();
    ...
    var typed = normalize(elements.input.value);
    ...
    if (typed === state.target || isTimedOut(metrics.elapsedSeconds)) {
  ...
  function startTimer() {
    ...
    state.timerId = window.setInterval(function () {
      ...
      var metrics = getMetrics(normalize(elements.input.value));
      updateStats(metrics);
      if (isTimedOut(metrics.elapsedSeconds)) {
        finishPractice(metrics);
      }
  ...
  function isTimedOut(elapsedSeconds) {
    return state.duration > 0 && elapsedSeconds >= state.duration;
  }
  ...
  function updateStats(metrics) {
    var displaySeconds = state.mode === "game"
      ? Math.max(60 - Math.floor(metrics.elapsedSeconds), 0)
      : state.duration > 0
        ? Math.max(state.duration - Math.floor(metrics.elapsedSeconds), 0)
  ...
  function renderHistory() {
    ...
    results.forEach(function (result) {
      ...
      appendHistoryCell(item, historyModeLabel(result.mode) + " · " + historyLanguageLabel(result.language), true);
  ...
  function historyLanguageLabel(value) {
    ...
    return value || data[state.language].label;
  }
  ...
  function setupInvader() {
    var words = sample(data[state.language].words, 28);
    state.gameState = { words: words, x: 0, y: 0, dir: 1, step: 0 };
  }

  function setupMining() {
    state.gameState = { words: sample(data[state.language].words, 25), cpu: 0, step: 0 };
  }
  ...
  function tickGame() {
    ...
    if (elapsed >= 60) {
      finishPractice(gameMetrics(elapsed));
      return;
    }
    if (state.game === "rain") {
      tickRain(elapsed);
  ...
  function gameMetrics(elapsed) {
    ...
    return {
      ...
      progress: Math.min((elapsed / 60) * 100, 100),
      score: state.score
    };
  }
  ...
  function tickRain() {
    ...
    if (gs.spawn % 7 === 0 && gs.tokens.length < 12) {
      gs.tokens.push({
        text: sample(data[state.language].words, 1)[0],
  ...
  function updateGameInput() {
    var typed = normalize(elements.input.value).trim();
    if (!typed) {
      return;
    }
    if (state.game === "rain") {
  ...
  function renderKeyboard() {
    ...
    keyboardRows.forEach(function (row) {
      ...
      row.forEach(function (key) {
        var code = key[0];
        var layoutLabel = languageKeyLabels[state.language][code];
        var primary = key[1];
        var secondary = key[2] || "";
  ...
  function charToCode(char) {
    ...
    if (state.language === "ru" && ruPunctuationMap[char]) {
