(function () {
  ...
  function init() {
    ...
    window.addEventListener("popstate", function () {
      setMode(modeFromPath(window.location.pathname) || "seat", false);
    });
    resetPractice();
    renderHistory();
  }
  ...
  function setMode(mode, updateURL) {
    ...
    if (updateURL && window.history && window.history.pushState) {
      var nextPath = pathForMode(mode);
      if (window.location.pathname !== nextPath) {
        window.history.pushState({ mode: mode }, "", nextPath);
      }
    }
    resetPractice();
  }
  ...
  function applyKeyboardVisibility() {
    ...
    elements.keyboardPanel.hidden = !state.keyboardVisible;
    if (elements.keyboardToggleButton) {
      elements.keyboardToggleButton.textContent = state.keyboardVisible ? "키보드 숨기기" : "키보드 보이기";
