(function () {
  ...
  function writePreferences(preferences) {
    remotePreference = Object.assign({}, remotePreference || {}, preferences || {}, { ready: true });
    saveRemotePreferences(preferences || {});
    try {
      window.localStorage.setItem(PREFERENCES_KEY, JSON.stringify(preferences));
    } catch (error) {
      // Browser cache may be unavailable; ClickHouse-backed preference saving is still attempted.
    }
  }
  ...
  function saveRemotePreferences(preferences) {
    var payload = {
      uiLanguage: normalizeLanguage(preferences.uiLanguage || preferences.nativeLanguage || currentPreferenceState().uiLanguage),
      nativeLanguage: normalizeLanguage(preferences.nativeLanguage || preferences.uiLanguage || currentPreferenceState().uiLanguage),
      practiceLanguage: normalizeLanguage(preferences.practiceLanguage || currentPreferenceState().practiceLanguage),
      gameDifficulty: Number.isFinite(Number(preferences.gameDifficulty)) ? Math.max(0, Math.min(10, Math.round(Number(preferences.gameDifficulty)))) : 3
    };
  ...
  function mergeRemoteLearningState(payload) {
    ...
    if (Array.isArray(payload.results)) {
      remoteLearningResults[key] = payload.results.slice();
    }
    renderLearningSummary();
    renderShellHistory();
    renderMyInfoRecords();
  ...
  function isMeaningfulResult(result) {
    ...
    var knownModes = ["course", "seat", "word", "sentence", "sentenceShort", "sentenceLong", "game", "keyboard", "언어 코스", "Language Course", "Языковой курс", "자리 연습", "단어 연습", "문장 연습", "단문 연습", "장문 연습", "타자게임", "키보드 보기"];
