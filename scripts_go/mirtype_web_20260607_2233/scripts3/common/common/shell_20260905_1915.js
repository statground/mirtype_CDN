(function () {
  ...
  var preferenceWriteSequence = 0;
  var remoteLearningStates = {};
  var remoteLearningResults = {};
  var remoteLearningRequests = {};
  var remoteLearningLoaded = {};
  var courseCompletionPosts = {};
  var supportedLanguages = ["ko", "en", "ru"];
  var learningActivityKeys = ["practice", "course", "game"];
  var FLAG_ASSET_VERSION = "20260630_2245";
  var FLAG_ASSET_VERSION_BY_COUNTRY = { us: "20260704_1013" };
  var FLAG_CDN_BASE = (function () {
    function mirrorBase(version, root) {
      var cleanRoot = String(root || "").replace(/^\/+|\/+$/g, "");
      if (!version || !cleanRoot) return "";
      return "/assets/mirtype-cdn/" + version + "/" + cleanRoot + "/assets/flags/";
    }
    var script = document.currentScript && document.currentScript.src ? document.currentScript.src : "";
    if (script) {
      return script.replace(/scripts3\/common\/common\/[^/]+$/, "assets/flags/");
    }
    ...
    if (marker) {
      ...
      if (version && root) {
        return mirrorBase(version, root);
      }
    }
    return mirrorBase(window.MirtypeCDNVersion || "", window.MirtypeAssetRoot || "scripts_go/mirtype_web_20260607_2233/") || "/assets/flags/";
  })();
  var languageOptions = {
  ...
  var shellLocales = {
    ko: {
      ...
      metaTitleRankings: "MirType 랭킹",
      metaDescriptionRankings: "내 언어와 연습할 언어, 활동 종류별 MirType 학습 순위",
      ...
      rankings: {
        mainAria: "MirType 랭킹",
        kicker: "Learning Rankings",
        title: "MirType 랭킹",
        summary: "내 언어와 연습할 언어, 활동별 기록을 모아 기간별 순위를 보여줍니다.",
        heroMetric: "주간 · 월간 · 연간 · 누적",
        boardKicker: "Leaderboard",
        boardTitle: "학습 순위",
    ...
    en: {
      ...
      metaTitleRankings: "MirType Rankings",
      metaDescriptionRankings: "MirType learning rankings by learner language, practice language, and activity",
      ...
      rankings: {
        mainAria: "MirType Rankings",
        kicker: "Learning Rankings",
        title: "MirType Rankings",
        summary: "Rankings grouped by learner language, practice language, activity, and period.",
        heroMetric: "Weekly · Monthly · Yearly · Total",
        boardKicker: "Leaderboard",
        boardTitle: "Learning Rankings",
    ...
    ru: {
      ...
      metaTitleRankings: "Рейтинг MirType",
      metaDescriptionRankings: "Рейтинг обучения MirType по языку пользователя, языку практики и активности",
      ...
      rankings: {
        mainAria: "Рейтинг MirType",
        kicker: "Learning Rankings",
        title: "Рейтинг MirType",
        summary: "Рейтинг по языку пользователя, языку практики, активности и периоду.",
        heroMetric: "Неделя · Месяц · Год · Всего",
        boardKicker: "Leaderboard",
        boardTitle: "Рейтинг обучения",
  ...
  function languageFlagURL(value) {
    ...
    var version = FLAG_ASSET_VERSION_BY_COUNTRY[country] || FLAG_ASSET_VERSION;
    return FLAG_CDN_BASE + country + "_" + version + ".svg";
  }
  ...
  function localizeRankingsMeta(copy) {
    if (!qs(".rankings-page")) {
      return;
    }
    document.title = copy.metaTitleRankings;
    var description = qs("meta[name='description']");
    var ogTitle = qs("meta[property='og:title']");
    var ogDescription = qs("meta[property='og:description']");
    if (description) {
      description.setAttribute("content", copy.metaDescriptionRankings);
    }
    if (ogTitle) {
      ogTitle.setAttribute("content", copy.metaTitleRankings);
    }
    if (ogDescription) {
      ogDescription.setAttribute("content", copy.metaDescriptionRankings);
    }
  }
  ...
  function writePreferences(preferences) {
    ...
    preferenceWriteSequence += 1;
    var sequence = preferenceWriteSequence;
    remotePreference = Object.assign({}, remotePreference || {}, payload, { ready: true });
    cachePreferences(payload);
    saveRemotePreferences(payload, sequence);
  }
  ...
  function saveRemotePreferences(preferences, sequence) {
    ...
    fetchJSON("/api/learning/preferences/", {
      ...
      if (saved && saved.ready) {
        if (typeof sequence === "number" && sequence < preferenceWriteSequence) {
          return;
        }
        if (shouldKeepCachedPreference(readCachedPreferences(), saved)) {
          return;
        }
    ...
    return fetchJSON("/api/learning/preferences/").then(function (preference) {
      ...
      if (!preference || !preference.ready) {
        if (hasPreferenceLanguages(cached)) {
          ...
          saveRemotePreferences(cachedPayload, preferenceWriteSequence);
      ...
      if (shouldKeepCachedPreference(cached, preference)) {
        ...
        saveRemotePreferences(payload, preferenceWriteSequence);
        requestLearningState();
        requestMyInfoLearningStates();
        return remotePreference;
      }
  ...
  function saveLanguagePreference(kind, language) {
    ...
    var previousPracticeLanguage = normalizeLanguage(preferences.practiceLanguage || preferences.targetLanguage || preferences.language || previousUiLanguage);
    if (kind === "ui") {
    ...
    if ((kind === "ui" && previousUiLanguage !== normalized) || (kind !== "ui" && previousPracticeLanguage !== normalized)) {
      try {
        window.dispatchEvent(new CustomEvent("mirtype:languagechange", {
          detail: {
            uiLanguage: normalizeLanguage(preferences.uiLanguage || preferences.nativeLanguage || normalized),
            practiceLanguage: normalizeLanguage(preferences.practiceLanguage || normalized)
          }
        }));
      } catch (err) {
  ...
  var languagePopoverReturnFocus = null;
  var languagePopoverBackground = [];

  function restoreLanguagePopoverBackground() {
    languagePopoverBackground.forEach(function (entry) {
      entry.node.inert = entry.inert;
    });
    languagePopoverBackground = [];
  }
  ...
  function setLanguagePopoverOpen(open) {
    ...
    if (open) {
      languagePopoverReturnFocus = document.activeElement;
      ...
      while (branch.parentElement && branch !== document.body) {
        Array.from(branch.parentElement.children).forEach(function (sibling) {
          if (sibling !== branch) {
            languagePopoverBackground.push({ node: sibling, inert: sibling.inert });
            sibling.inert = true;
          }
        });
      ...
      restoreLanguagePopoverBackground();
    }
    popover.hidden = !open;
    qsa("[data-ui-language-toggle]").forEach(function (button) {
    ...
    if (open) {
      var first = popover.querySelector("[data-ui-language-close]");
      if (first) first.focus();
    } else if (languagePopoverReturnFocus && languagePopoverReturnFocus.isConnected) {
      languagePopoverReturnFocus.focus();
      languagePopoverReturnFocus = null;
    }
  }

  function trapLanguagePopoverFocus(event) {
    var popover = qs("#uiLanguagePopover");
    if (!popover || popover.hidden || event.key !== "Tab") return;
    var focusable = Array.from(popover.querySelectorAll('button:not([disabled]), a[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex="0"]')).filter(function (node) {
  ...
  function normalizeLearningActivityKey(value, fallback) {
    var key = String(value || "").trim();
    if (learningActivityKeys.indexOf(key) >= 0) {
      return key;
    }
    return fallback || "";
  }
  ...
  function currentActivityKey() {
    ...
    var scopedSummary = qs("[data-learning-summary][data-learning-activity]");
    if (scopedSummary) {
      return normalizeLearningActivityKey(scopedSummary.getAttribute("data-learning-activity"), "");
    }
    return "";
  }
  ...
  function requestLearningState(options) {
    ...
    if (remoteLearningLoaded[cacheKey] && !remoteLearningStates[cacheKey]) {
    ...
    remoteLearningRequests[cacheKey] = fetchJSON("/api/learning/state/?" + params.toString()).then(function (state) {
      remoteLearningLoaded[cacheKey] = true;
  ...
  function remoteStateLoadedForScope(options) {
    var config = options || {};
    var pairKey = config.pairKey || currentLanguagePairKey();
    var activityKey = Object.prototype.hasOwnProperty.call(config, "activityKey") ? config.activityKey : currentActivityKey();
    return !!remoteLearningLoaded[scopeCacheKey(pairKey, activityKey)];
  }

  function learningSummaryActivity(root) {
    if (root && root.getAttribute) {
      if (!root.hasAttribute("data-learning-activity")) {
        return currentActivityKey();
      }
      return normalizeLearningActivityKey(root.getAttribute("data-learning-activity"), currentActivityKey() || "practice");
    }
    return currentActivityKey();
  }
  ...
  function calculateLearningSummary(options) {
    ...
    var hasRemoteOverall = remoteState && (
      Object.prototype.hasOwnProperty.call(remoteState, "overallSessionCount") ||
      Object.prototype.hasOwnProperty.call(remoteState, "overallStreakDays")
    );
    if (remoteState && (remoteState.ready || hasRemoteOverall)) {
      var sessions = Number(remoteState.sessionCount) || 0;
      var displaySessions = Number.isFinite(Number(remoteState.overallSessionCount)) ? Number(remoteState.overallSessionCount) : sessions;
      var displayStreak = Number.isFinite(Number(remoteState.overallStreakDays)) ? Number(remoteState.overallStreakDays) : Number(remoteState.streakDays);
      ...
      return {
        ...
        sessions: Math.max(0, displaySessions || 0),
        streak: Math.max(0, displayStreak || 0),
    ...
    if (!results.length && !remoteStateLoadedForScope(options)) {
      return {
        pending: true,
        level: 1,
        sessions: 0,
  ...
  function courseBundlesForLanguage(language) {
    ...
    var key = normalizeLanguage(language);
    var local = sanitizeCourseBundlesForLanguage(courseBundles[key] || [], key);
    var external = sanitizeCourseBundlesForLanguage(externalBundles[key] || [], key).map(function (bundle) {
      return Object.assign({}, bundle, { external: true });
    });
    if (local.length) {
      return external.length ? local.concat(external) : local;
    }
    if (external.length) {
      return external;
    }
    return key === "ko" ? sanitizeCourseBundlesForLanguage(courseBundles.ko || [], "ko") : [];
  }
  ...
  function rotateCourseItems(items, offset) {
    var source = Array.isArray(items) ? items.filter(function (item) {
      return !!courseItemText(item);
    }) : [];
    if (!source.length) {
      return [];
    }
    var start = Math.abs(offset || 0) % source.length;
    return source.slice(start).concat(source.slice(0, start));
  }

  function courseVariantSeed(summary, bundle, dayIndex) {
    return hashString([
      dayKey(new Date()),
      normalizeLanguage(currentPreferenceState().practiceLanguage),
      (bundle && (bundle.index || bundle.name)) || "",
  ...
  function generatedCourseSentences(language, words, seed) {
    var cleanWords = rotateCourseItems(words || [], seed).map(courseItemText).filter(function (word) {
      return courseTextMatchesLanguage(language, word);
    });
    if (cleanWords.length < 2) {
      return [];
    }
    function word(index) {
      return cleanWords[index % cleanWords.length];
    }
    if (language === "ko") {
      return [
        word(0) + "와 " + word(1) + "을 오늘 문장으로 연결합니다.",
        word(2) + "을 말한 뒤 " + word(3) + "을 다시 확인합니다.",
        word(4) + "에서 " + word(5) + "까지 천천히 입력합니다."
      ];
    }
    if (language === "ru") {
      return [
        "Сегодня я соединяю " + word(0) + " и " + word(1) + ".",
        "Я повторяю " + word(2) + " и " + word(3) + " в новой фразе.",
        "После слова " + word(4) + " я снова пишу " + word(5) + "."
      ];
    }
    return [
      "Today I connect " + word(0) + " with " + word(1) + ".",
      "I use " + word(2) + " and " + word(3) + " in a new sentence.",
      "After " + word(4) + ", I review " + word(5) + " slowly."
    ];
  }

  function appendUniqueCourseItems(items, additions, language, limit) {
    var seen = {};
    var out = [];
    (items || []).concat(additions || []).forEach(function (item) {
      var text = courseItemText(item);
      var key = text.toLowerCase();
      if (!text || seen[key] || !courseTextMatchesLanguage(language, text)) {
        return;
      }
      seen[key] = true;
      out.push(item);
  ...
  function variedCourseDay(day, language, summary, bundle, dayIndex, sharedSeed) {
    var source = day || {};
    var seed = Number.isFinite(sharedSeed) ? sharedSeed : courseVariantSeed(summary, bundle, dayIndex);
    var words = rotateCourseItems(source.words || [], seed);
    var baseSentences = rotateCourseItems(source.sentences || [], seed + 3);
    var generated = baseSentences.length ? [] : generatedCourseSentences(language, words, seed + 7);
    return Object.assign({}, source, {
      words: words,
      sentences: appendUniqueCourseItems(baseSentences, generated, language, 5),
      longs: rotateCourseItems(source.longs || [], seed + 11)
    });
  }

  function coursePositionForStep(bundles, absoluteStep) {
    var total = (bundles || []).reduce(function (sum, bundle) {
      return sum + (bundle.days || []).length;
    }, 0);
    var requested = Number(absoluteStep);
    var absolute = Math.min(Math.max(0, total - 1), Math.max(0, Number.isFinite(requested) ? Math.floor(requested) : 0));
    var remaining = absolute;
    for (var index = 0; index < (bundles || []).length; index += 1) {
      var count = (bundles[index].days || []).length;
      if (remaining < count) {
        return { bundleIndex: index, dayIndex: remaining, absoluteStep: absolute, totalSteps: total };
      }
      remaining -= count;
    }
    return { bundleIndex: -1, dayIndex: -1, absoluteStep: 0, totalSteps: 0 };
  }

  function courseNativeBundle(bundle, bundleIndex, nativeBundles) {
    var sourceIndex = Number(bundle && bundle.index);
    if (Number.isFinite(sourceIndex) && sourceIndex > 0) {
      return (nativeBundles || []).find(function (candidate) {
        return Number(candidate.index) === sourceIndex;
      }) || {};
    }
    // Older cached API payloads have no source index. Their translated arrays
    // can differ in length, so they must not inherit an unrelated positional title.
    if (bundle && bundle.external) {
      return {};
    }
    var positional = (nativeBundles || [])[bundleIndex];
    return positional && !positional.external && !(Number(positional.index) > 0) ? positional : {};
  }

  function courseNativeDay(day, dayIndex, nativeBundle) {
    var days = nativeBundle.days || [];
    var sourceIndex = Number(day && day.index);
    if (Number.isFinite(sourceIndex) && sourceIndex > 0) {
      return days.find(function (candidate) {
        return Number(candidate.index) === sourceIndex;
      }) || {};
    }
    var positional = days[dayIndex];
    return positional && !(Number(positional.index) > 0) ? positional : {};
  }
  ...
  function courseBundleForSummary(summary) {
    ...
    var daysPerLevel = Math.max(1, Number(summary.daysPerLevel) || 7);
    var absoluteStep = Number.isFinite(summary.absoluteStep) ? summary.absoluteStep :
      Math.max(0, (Number(summary.level) || 1) - 1) * daysPerLevel + Math.max(0, Number(summary.currentStep) || 0);
    var position = coursePositionForStep(bundles, absoluteStep);
    var bundleIndex = position.bundleIndex;
    var bundle = bundles[bundleIndex] || {};
    var dayIndex = position.dayIndex;
    var day = (bundle.days || [])[dayIndex] || {};
    var nativeBundles = courseBundlesForLanguage(nativeLanguage);
    var nativeBundle = courseNativeBundle(bundle, bundleIndex, nativeBundles);
    var nativeDay = courseNativeDay(day, dayIndex, nativeBundle);
    var seed = courseVariantSeed(summary, bundle, dayIndex);
    var variedDay = variedCourseDay(day, language, summary, bundle, dayIndex, seed);
    var variedNativeDay = variedCourseDay(nativeDay, nativeLanguage, summary, nativeBundle, dayIndex, seed);
    return {
      bundleIndex: bundleIndex,
      dayIndex: dayIndex,
      bundle: bundle,
      day: variedDay,
      dayNumber: Math.max(1, dayIndex + 1),
      dayCount: (bundle.days || []).length,
      absoluteStep: position.absoluteStep,
      totalSteps: position.totalSteps,
      language: language,
      nativeLanguage: nativeLanguage,
      nativeBundle: nativeBundle,
      nativeDay: variedNativeDay
    };
  }
  ...
  function sanitizeCourseBundlesForLanguage(bundles, language) {
    return (bundles || []).map(function (bundle) {
      if (!courseBundleMatchesLanguage(bundle, language)) {
        return null;
      }
  ...
  function refreshCourseTypingProgress(courseCopy) {
    ...
    recordInlineCourseCompletion(done, total);
  }

  function inlineCourseCompletionKey(active, total) {
    var bundle = (active && active.bundle) || {};
    var day = (active && active.day) || {};
    return [
      currentLanguagePairKey(),
      "course",
      (active && active.language) || currentPreferenceState().practiceLanguage,
      String((active && active.bundleIndex) || 0),
      String((active && active.dayIndex) || 0),
      String((active && active.dayNumber) || 1),
      dayKey(new Date()),
      String(bundle.name || ""),
      String(day.title || ""),
      String(total || 0)
  ...
  function recordInlineCourseCompletion(done, total) {
    if (!total || done < total) {
      return;
    }
    ...
    var key = inlineCourseCompletionKey(active, total);
    if (courseCompletionPosts[key]) {
      return;
    }
    courseCompletionPosts[key] = true;
    var prefs = currentPreferenceState();
    var practiceLanguage = normalizeLanguage((active && active.language) || prefs.practiceLanguage);
    var uiLanguage = normalizeLanguage(prefs.uiLanguage);
    postLearningResult({
      mode: "course",
      game: "",
      activityKey: "course",
      activityDetailKey: "course",
      languagePairKey: languagePairKey(uiLanguage, practiceLanguage),
      activityType: "course",
      uiLanguage: uiLanguage,
      nativeLanguage: uiLanguage,
      practiceLanguage: practiceLanguage,
      ...
      elapsedSeconds: 0,
      score: total,
      gameDifficulty: Number(prefs.gameDifficulty) || 3,
  ...
  function renderCourseBundle(summary, courseCopy) {
    ...
    var active = courseBundleForSummary(summary);
    var displayBundle = active.nativeBundle && active.nativeBundle.name ? active.nativeBundle : active.bundle || {};
    var displayDay = active.nativeDay && (active.nativeDay.title || active.nativeDay.summary) ? active.nativeDay : active.day || {};
    setText("[data-course-difficulty-title]", formatCopyValues(courseCopy.bundleTitle || "%s · Day %d / %d", [displayBundle.name || (active.bundle && active.bundle.name) || courseCopy.bundleFallbackTitle, active.dayNumber, active.dayCount]));
    ...
    setText("[data-course-bundle-summary]", displayDay.summary || (active.day && active.day.summary) || courseCopy.bundleSummaryFallback || "");
    setText("[data-course-day-title]", displayDay.title || (active.day && active.day.title) || courseCopy.bundleFallbackTitle || "");
  ...
  function renderProgressSummary(root, summary, progressCopy) {
    var pending = !!(summary && summary.pending);
    var mount = root || document;
    if (mount && mount.setAttribute) {
      mount.setAttribute("data-learning-state", pending ? "loading" : "ready");
      mount.setAttribute("aria-busy", pending ? "true" : "false");
    }
    if (pending) {
      setTextWithin(root, "[data-progress-level]", formatCopy(progressCopy.levelValue, "..."));
      setTextWithin(root, "[data-progress-streak]", "...");
      setTextWithin(root, "[data-progress-streak-unit]", progressCopy.streakUnit);
      setTextWithin(root, "[data-progress-next-count]", "...");
      setTextWithin(root, "[data-progress-next-unit]", progressCopy.nextUnit || progressCopy.sessionUnit);
      setTextWithin(root, "[data-progress-session-count]", "...");
      setTextWithin(root, "[data-progress-session-unit]", progressCopy.sessionUnit);
      qsa("[data-progress-fill]", root || document).forEach(function (node) {
        node.style.width = "0%";
      });
      return;
    }
    setTextWithin(root, "[data-progress-level]", formatCopy(progressCopy.levelValue, summary.level));
    setTextWithin(root, "[data-progress-streak]", String(summary.streak));
    setTextWithin(root, "[data-progress-streak-unit]", progressCopy.streakUnit);
    setTextWithin(root, "[data-progress-next-count]", String(summary.nextCount));
    setTextWithin(root, "[data-progress-next-unit]", progressCopy.nextUnit || progressCopy.sessionUnit);
    setTextWithin(root, "[data-progress-session-count]", String(summary.sessions));
    setTextWithin(root, "[data-progress-session-unit]", progressCopy.sessionUnit);
    qsa("[data-progress-fill]", root || document).forEach(function (node) {
  ...
  function renderLearningSummary(copy) {
    ...
    var progressRoots = qsa("[data-learning-summary]");
    if (progressRoots.length) {
      progressRoots.forEach(function (root) {
        var activityKey = learningSummaryActivity(root);
        var summaryOptions = activityKey ? {
          activityKey: activityKey,
          activityDetailKey: activityKey
        } : {
          activityKey: ""
        };
        renderProgressSummary(root, calculateLearningSummary(summaryOptions), progressCopy);
      });
    } else {
      renderProgressSummary(document, calculateLearningSummary(), progressCopy);
    }
    if (qs("[data-course-prev-level]") || qs("[data-course-node]") || qs("[data-course-bundle]")) {
      var courseSummary = calculateLearningSummary({
        activityKey: "course",
        activityDetailKey: "course"
      });
      setText("[data-course-prev-level]", formatCopy(progressCopy.levelValue, Math.max(0, courseSummary.level - 1)));
      setText("[data-course-next-level]", formatCopy(progressCopy.levelValue, courseSummary.level + 1));
      setText("[data-course-level-caption]", courseLevelCaption(courseSummary, courseCopy));
      updateCourseNodes(courseSummary);
      renderCourseBundle(courseSummary, courseCopy);
    }
  }
  ...
  ready(function () {
    ...
    document.addEventListener("keydown", function (event) {
      trapLanguagePopoverFocus(event);
      if (event.key === "Escape") {
        closeMobileMenu();
        setLanguagePopoverOpen(false);
        setShellSettingsOpen(false);
