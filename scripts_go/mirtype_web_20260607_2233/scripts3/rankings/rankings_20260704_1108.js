(function () {
  ...
  var flagAssetVersion = "20260630_2245";
  var flagAssetVersionByCountry = { us: "20260704_1013" };
  ...
  var locales = {
    ko: {
      ...
      requestError: "랭킹을 잠시 불러오지 못했습니다.",
	      ...
	      myRanking: "내 랭킹",
	      myRankingEmpty: "아직 내 랭킹 기록이 없습니다.",
	      topFive: "상위 5위",
	      period: "기간",
    ...
    en: {
      ...
      slowLoading: "Preparing ranking totals.",
	      ...
	      myRankingEmpty: "No ranking record for you yet.",
	      topFive: "Top 5",
	      period: "Period",
    ...
    ru: {
      ...
      slowLoading: "Готовим итоги рейтинга.",
	      ...
	      myRanking: "Мой рейтинг",
	      myRankingEmpty: "У вас пока нет записи в рейтинге.",
	      topFive: "Топ-5",
	      period: "Период",
  ...
  function flagHTML(code) {
    ...
    if (!country) return "";
    var version = flagAssetVersionByCountry[country] || flagAssetVersion;
    return '<img class="ranking-flag" src="' + flagAssetBase + country + "_" + version + '.svg" alt="" loading="lazy" decoding="async">';
  }
  ...
  function metricHTML(label, value) {
    return '<span class="ranking-metric"><small>' + escapeHTML(label) + '</small><strong>' + escapeHTML(value) + "</strong></span>";
	  ...
	  function rowHTML(item, options) {
	    ...
	    var className = "ranking-row";
	    if (options.mine || item.isCurrent) className += " is-my-ranking";
	    var metrics = [
	  ...
	  function myRankingHTML(payload) {
	    var t = copy();
	    var item = payload && payload.myRanking;
	    if (!item) {
	      return '<section class="my-ranking-panel is-empty">' +
	        "<strong>" + escapeHTML(t.myRanking) + "</strong>" +
	        "<p>" + escapeHTML(t.myRankingEmpty) + "</p>" +
	      "</section>";
	    }
	    return '<section class="my-ranking-panel">' +
	      "<strong>" + escapeHTML(t.myRanking) + "</strong>" +
	      rowHTML(item, { mine: true }) +
	    "</section>";
	  }

	  function listHTML(payload) {
	    var t = copy();
	    var items = payload && Array.isArray(payload.items) ? payload.items.slice(0, 5) : [];
	    var heading = '<div class="ranking-list-head"><strong>' + escapeHTML(t.topFive) + "</strong></div>";
	    if (!items.length) {
	      return '<section class="ranking-top-section">' + heading + '<div class="ranking-empty">' + escapeHTML(t.empty) + "</div></section>";
	    }
	    return '<section class="ranking-top-section">' + heading + '<div class="ranking-list">' + items.map(function (item) {
	      return rowHTML(item);
	    }).join("") + "</div></section>";
	  }
	  ...
	  function render(payload) {
	    ...
	    root.innerHTML = controlsHTML() + myRankingHTML(lastPayload) + listHTML(lastPayload);
	    bindControls();
	  }
