(function () {
  ...
  function load() {
    ...
    if (state.mode === "detail") {
      return postJSON("/books/api/detail/", { language: state.language, isbn: state.isbn }).then(renderDetail).catch(renderError);
    }
    if (state.mode === "list") {
      return postJSON("/books/api/list/", {
        ...
        query: state.query,
        page: state.page
      }).then(renderList).catch(renderError);
    }
    return postJSON("/books/api/landing/", {}).then(renderLanding).catch(renderError);
  }
