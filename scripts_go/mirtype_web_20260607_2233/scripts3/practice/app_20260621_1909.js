(function () {
  ...
  function buildCourseItems() {
    ...
    var items = [];
    (day.words || []).forEach(function (item, index) {
      var next = makeCourseItem("word", item, index, nativeDay.words || [], active);
      if (next) {
        items.push(next);
      }
    });
    (day.sentences || []).forEach(function (item, index) {
    ...
    if (Array.isArray(day.longs)) {
      day.longs.forEach(function (item, index) {
        var next = makeCourseItem("long", item, index, nativeDay.longs || [], active);
        if (next) {
          items.push(next);
        }
      });
    } else if ((day.sentences || []).length > 1) {
      var longText = (day.sentences || []).map(courseItemText).filter(Boolean).join("\n");
    ...
    if (!items.length) {
      courseLettersFromWords((day.words || []).length ? day.words : practiceData().words).slice(0, 5).forEach(function (letter) {
  ...
  function appendCoursePronunciationRows(panel, copy, item) {
    ...
    var count = 0;
    if (pronunciations.ipa) {
      appendCourseAssistRow(panel, copy.coursePronunciationIpaLabel || "IPA", pronunciations.ipa);
      count += 1;
    }
    if (pronunciations.romanization) {
      appendCourseAssistRow(panel, copy.coursePronunciationRomanLabel || "Latin", pronunciations.romanization);
      count += 1;
    }
    if (pronunciations.native) {
      appendCourseAssistRow(panel, courseNativePronunciationLabel(copy), pronunciations.native);
      count += 1;
    }
    if (!count && item && item.pronunciation) {
      appendCourseAssistRow(panel, copy.coursePronunciationLabel || "Pronunciation", item.pronunciation);
      count += 1;
    }
    return count;
  }
  ...
  function applyPracticeContentPayload(payload) {
    ...
    ["ko", "en", "ru"].forEach(function (language) {
      ...
      var words = contentList(external.words);
      var shorts = contentList(external.shorts);
      var longs = contentList(external.longs);
